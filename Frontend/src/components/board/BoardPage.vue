<template>
  <ul class="pagination justify-content-center" v-if="$route.query.no">
    ...
    >
      <router-link
        :to="`/board/list?no=${(startPageIndex + index - 1) * listRowCount}`"
        class="page-link"
    ...
    <li class="page-item" v-if="next">
      <router-link
        :to="`/board/list?no=${(endPageIndex + 1) * listRowCount}`"
        class="page-link"
        aria-label="Next"
...
export default {
  ...
  mounted() {
    // this.$router.push("board/list?no=" + this.listRowCount);
    this.$router.push({
      name: "boardList",
      query: { no: this.listRowCount },
    });
  },
};
</script>
